...
func before_each():
	...
	_event_token = EventBus.on(
		EventCatalog.BOSS_HEALTH_CHANGED, Callable(self, "_on_boss_health_changed")
	)
...
func test_player_attack_damages_boss_and_fires_event():
	...
	assert_lt(
		final_boss_health,
		initial_boss_health,
		"Boss health should be lower after being hit by a player melee attack."
	)
	assert_true(
		_boss_health_changed_event_fired,
		"A BOSS_HEALTH_CHANGED event should be emitted on the global EventBus."
	)

	assert_gt(
		_player.entity_data.determination_counter,
		0,
		"Player determination should increase after dealing damage."
	)
