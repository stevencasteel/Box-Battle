...
func before_each():
	...
	var dependencies = {
		"data_resource": _player_data, "config": CombatConfig, "fx_manager": FakeFXManager.new()  # Inject the fake
	}
...
func after_each():
	if (
		...
		and _health_component.died.is_connected(_on_health_component_died)
	):
...
func test_cannot_damage_when_invincible():
	...
	assert_eq(
		_player_data.health,
		_player_data.max_health - 1,
		"Health should not have changed on second hit."
	)
