...
func before_each():
	...
	_health_component = _player.health_component
...
func test_initial_health_is_max_health():
	assert_eq(_player.entity_data.health, _player.entity_data.max_health, "Health should be max at start.")

func test_apply_damage_reduces_health():
	var initial_health = _player.entity_data.health
	...
	assert_eq(_player.entity_data.health, initial_health - 2, "Health should be reduced by 2.")
...
func test_cannot_damage_when_invincible():
	...
	assert_eq(_player.entity_data.health, _player.entity_data.max_health - 1, "Health should not have changed on second hit.")

func test_died_signal_emitted_when_health_is_zero():
	var damage_info = DamageInfo.new()
	damage_info.amount = _player.entity_data.max_health
	damage_info.source_node = _damage_source
	...
	assert_eq(_player.entity_data.health, 0, "Health should be 0 after lethal damage.")
