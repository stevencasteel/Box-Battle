...
# --- Constants ---
const PlayerResourceComponent = preload(
	"res://src/entities/player/components/player_resource_component.gd"
)
...
func before_each():
	...
	var dependencies = {"data_resource": _player_data, "event_bus": _fake_event_bus}
...
func test_on_damage_dealt_increments_determination():
	...
	assert_eq(
		...
		initial_determination + 1,
		"Determination should increment by 1."
	)
...
func test_healing_charge_gained_at_threshold():
	...
	assert_eq(
		_player_data.determination_counter,
		0,
		"Determination should reset to 0 after gaining a charge."
	)
...
func test_healing_charges_are_capped():
	...
	assert_eq(
		_player_data.healing_charges,
		CombatConfig.player_max_healing_charges,
		"Healing charges should not exceed the max."
	)
	assert_eq(
		_player_data.determination_counter,
		CombatConfig.player_determination_per_charge - 1,
		"Determination should not increment if charges are already at max."
	)
...
func test_event_emitted_when_charge_gained():
	...
	assert_true(
		_fake_event_bus.was_event_emitted(EventCatalog.PLAYER_HEALING_CHARGES_CHANGED),
		"Should emit PLAYER_HEALING_CHARGES_CHANGED event."
	)

	var payload: PlayerHealingChargesChangedEvent = _fake_event_bus.get_payload_for_event(
		EventCatalog.PLAYER_HEALING_CHARGES_CHANGED
	)
	assert_not_null(payload, "Event payload should not be null.")
	assert_eq(
		payload.current_charges, 1, "Event payload should contain the correct new charge count."
	)
