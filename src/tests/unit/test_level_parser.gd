...
func test_parses_correct_dimensions():
	assert_eq(
		_build_data.dimensions_tiles, Vector2i(5, 3), "Grid dimensions should be parsed correctly."
	)
...
func test_parses_spawn_positions():
	...
	assert_eq(
		_build_data.player_spawn_pos,
		expected_player_pos,
		"Player spawn position should be parsed correctly."
	)
	assert_eq(
		_build_data.boss_spawn_pos,
		expected_boss_pos,
		"Boss spawn position should be parsed correctly."
	)
...
func test_parses_background_tiles():
	...
	assert_eq(
		_build_data.background_tiles.size(),
		3,
		"Should correctly identify all non-terrain/entity tiles as background."
	)
