...
# This version includes an `_is_dead` flag to definitively prevent any
# "zombie" actions after the boss has been defeated.
class_name BaseBoss
...
# NEW: State flag to prevent all actions after death.
...
func _ready():
	...
	phase_2_patterns = [single_shot_p2, volley_shot_p2]
	var volley_shot_p3 = volley_shot_p2.duplicate(); volley_shot_p3.cooldown *= 0.8
	phase_3_patterns = [volley_shot_p3]
...
func fire_shot_at_player():
	# THE FIX: This guard clause now prevents all zombie actions.
	if _is_dead: return
